...
| Pri | Pantalla | Archivo | Qué cambiar | Impacto |
|---|---|---|---|---|
...
| P1 | Admin | `admin/*` | Chrome con nav de secciones, tablas (estado pill), forms 640px, mensajes error/éxito, login centrado **sin** float WA. | Operación |
...
| P1 | Home productos/testimonios | `index.astro` + `base.css` | Hover translateY(-4px) 0.3s, borde `#ced7e0`, link line-hover. | Pulido |
...
**Sí:** Lenis ≥768 (ya), fadeIn secciones (ya), SplitText títulos (ya), chip del botón (ya, con **una** flecha), scale imagen blog, line-hover, shine en thumb de post, hamburger, GO TOP bounce. `prefers-reduced-motion: reduce` apaga todo.
